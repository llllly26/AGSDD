...
def dataset_argument(n=0):
    ...
    if n == 0:
        ...
        dataset_arg['divide_num'] = 1
        dataset_arg['divide_idx'] = 0
        dataset_arg['c_alpha_max_neighbors'] = 30  
...
def get_stat(graph_root, limited_num=None, num_subgroup=1000, max_limits=100000):
    ...
    wrong_proteins = []
    filenames = []
    # t_dirs = ['train', 'validation', 'test']
    # for dir in t_dirs:
    #     file = os.listdir(graph_root + dir)
    #     filenames += file
    ...
    # initialize mean, std
    graphi = torch.load(os.path.join(graph_root, filenames[0]))
    #keys: edge_attr; edge_dist; mu_r_norm; distances; edge_index; x; ss; pos.
    # graphi = torch.load(os.path.join(graph_root+'train', os.listdir(graph_root+'train')[0]))
    x, pos, mu_r_norm, edge_attr = graphi.x, graphi.pos, graphi.mu_r_norm, graphi.edge_attr    # 这些需要norm.
    ...
    for i in tqdm(range(n)):
        file = filenames[i]
        # root_dirs = ['train', 'validation', 'test']
        # for root in root_dirs:
        #     if file in os.listdir(graph_root+root):
        #         graph = torch.load(os.path.join(graph_root+root, file))
        #         break
        #     else:
        #         continue
    ...
    filename = 'mean_attr_CATH_train_N_30'
...
class NormalizeProtein(BaseTransform):
    ...
    def __init__(self, filename, skip_x=20, skip_edge_attr=64, safe_domi=1e-10):
        ...
        self.x_mean = dic['x_mean']   # 1.  one-hot.
        self.x_std = dic['x_std']
        self.pos_mean = dic['pos_mean']  # 2. pos.
        self.pos_std = torch.mean(dic['pos_std'])
        self.mu_r_norm_mean = dic['mu_r_norm_mean']  # 3. r_norm.
        self.mu_r_norm_std = dic['mu_r_norm_std']
        self.edge_attr_mean = dic['edge_attr_mean']  # 4. Edge.
