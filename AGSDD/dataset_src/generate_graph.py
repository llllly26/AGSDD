import os
import sys
sys.path.append('/home/ubuntu/wcl/GraDe')
...
def get_struc2ndRes(pdb_filename):
    ...
    p = PDBParser()
    structure = p.get_structure('random_id', pdb_filename)  # get_structure.
    model = structure[0]
    dssp = DSSP(model, pdb_filename, dssp='/home/ubuntu/anaconda3/envs/wcl39/bin/mkdssp')
...
if __name__ == '__main__':
    ...
    for key in ['training_set']:
        ...
        pdb_dir =f'/home/ubuntu/wcl/dataset/{key}/pdb/'

        save_dir = f'/home/ubuntu/wcl/dataset/Norm_train_N_30/test/'
        normalize_path = f'./dataset/train_mean_attr.pt'  #
        ...
        for filename in tqdm(filename_list):
            ...
            if os.path.exists(save_dir+filename.replace('.pdb','.pt')):
                ...
                try:
                    graph = pdb2graph(pdb_dir+filename, normalize_path)
                    if graph:
                        torch.save(graph,save_dir+filename.replace('.pdb','.pt'))
            
                        # print(save_dir+filename+"finished.")
                    else:
