...
class Cath_imem(InMemoryDataset):
    ...
    def __init__(self, root: str, name: str,
                 ...
                 micro_radius: int = 20,
                 c_alpha_max_neighbors: int = 10,  # 30.
                 cutoff: int = 30,
                 seq_dist_cut: int = 64,
...
#             if name in ['1bteA00', '1byiA00','1c1yB00']:
    ...
    def get_node_features(self, n_coords, c_coords, c_alpha_coords, coord_mask, with_coord_mask=True, use_angle=False, use_omega=False):
        ...
        if use_omega:
            ...
            num_angle_type = 2
            angles = np.zeros((num_res, num_angle_type))  # using 2.
            for i in range(num_res-1):
                # These angles are called φ (phi) which involves the backbone atoms C-N-Cα-C
                angles[i, 0] = dihedral(
                    c_coords[i], n_coords[i], c_alpha_coords[i], n_coords[i+1])
    ...
    def get_calpha_graph(self, rec, c_alpha_coords, n_coords, c_coords, coords, struc_2nd_res = None):
        ## coords is the 3D coordinates of the whole protein, shape is: \
        ## [num_residue, num_atom[8], 3]. 
        ## c_alpha_coords is the 3D coordinates of the alpha carbon of the whole protein;
        ## n_coords is the 3D coordinates of the N atom of the whole protein;
        ## c_coords is the 3D coordinates of the C atom of the whole protein;
        ## struc_2nd_res is the structure of the 2nd residue [one-hot] \
        ## if it is not None, then we will use it to get the 2nd residue features \
        ## otherwise, we will use the 1st residue features.
        ...
        # scalar_feature is dihedral angles of phi and psi, calculate {sin, cos} respectively. # [N, 4] 
        ...
        for i, residue in enumerate(rec.get_residues()):
            n_coord = n_coords[i]  # N
            c_alpha_coord = c_alpha_coords[i]   # C_α
            c_coord = c_coords[i]   # C
            ...
            v_i = np.cross(n_i, u_i)   # orthogonal to main chain
            assert (math.fabs(
        ...
        for i in range(num_residues):
            dst = list(np.where(distances[i, :] < self.cutoff)[0])  # index.
            dst.remove(i)
            if self.c_alpha_max_neighbors != None and len(dst) > self.c_alpha_max_neighbors:  # 如果超过30，取30.不足就保留.
            ...
            diff_vecs = residue_representatives_loc_feat[src, :] - residue_representatives_loc_feat[
                dst, :]  # (neigh_num, 3) C-alpha
            mean_vec = weights.dot(diff_vecs)  # (sigma_num, 3)  matmul.
        ...
        if x is not None:
            graph = Data(
                ...
                edge_attr=self.get_edge_features(  # calculate Edge attr.
            ...
            for i in range(len(dist_list)):
                ...
                basis_matrix = np.stack(
                    (n_i_feat[dst, :], u_i_feat[dst, :], v_i_feat[dst, :]), axis=0) # [3,3]
                p_ij = np.matmul(basis_matrix,
                                residue_representatives_loc_feat[src, :] - residue_representatives_loc_feat[
                                    dst, :])  # C_alpha coordinate.
                # [3,3].[3] -> [3,3].[3,1]=[3,1]->[3]
                q_ij = np.matmul(basis_matrix, n_i_feat[src, :])  # shape (3,) 法向量
                k_ij = np.matmul(basis_matrix, u_i_feat[src, :])  # N-C_α
                t_ij = np.matmul(basis_matrix, v_i_feat[src, :])  # 切向量
                s_ij = np.concatenate(
                    (p_ij, q_ij, k_ij, t_ij), axis=0)  # shape (12,)
                edge_feat_ori_list.append(s_ij)

            edge_feat_ori_feat = np.stack(
                edge_feat_ori_list, axis=0)  # shape (num_edges, 12)
            edge_feat_ori_feat = torch.from_numpy(
                edge_feat_ori_feat.astype(np.float32))

            graph.edge_attr = torch.cat(
                [graph.edge_attr, edge_feat_ori_feat], axis=1)  # (num_edges, 81+12)
    ...
    def distance_featurizer(self, dist_list, divisor) -> torch.Tensor:
        ...
        length_scale_list = [1.5 ** x for x in range(15)]  # 15 gaussian.
