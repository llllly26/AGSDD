...
class EGNN_NET(torch.nn.Module):
    def __init__(self, input_feat_dim, hidden_channels, edge_attr_dim,  dropout=0.0, n_layers=1, output_dim = 20,
        ...
        self.label_embedding = nn.Linear(20, embedding_dim)
        ####ss###

    def forward(self, data,time, gt=None, saved_dirs=None): 
        ...
        ##one-hot label.##
        labels = torch.arange(20).cuda()
        one_hot_tensor = F.one_hot(labels, num_classes=20).float()
        labels_embed = self.label_embedding(one_hot_tensor)  # [20,128]
        all_layer_attn_loss = 0
        ...
        for i, layer in enumerate(self.mpnn_layes):
            ...
            if self.update_edge:
                ...
                h,edge_attr, attn_loss = layer(x, edge_index, edge_attr, batch, labels_embed, gt=gt, layer_num=i, saved_dirs=saved_dirs)
                all_layer_attn_loss += attn_loss
            else:
        ...
        if self.embed_ss == -1:   # extra_x embedding also adding here.
            x = x+ss_embed 

        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.lin(x)

        return x, all_layer_attn_loss / 6
...
class DiscreteUniformTransition:
    ...
    def get_Qt_bar(self, alpha_bar_t, device):
        ...
        self.u_x = self.u_x.to(device)  # [1, 20, 20] uniform probability-0.05..
...
class AGSDD(nn.Module):
    ...
    def sample_discrete_feature_noise(self,limit_dist ,num_node):
        ...
        x_limit = limit_dist[None,:].expand(num_node,-1)
    ...
    def compute_batched_over0_posterior_distribution(self,X_t,Q_t,Qsb,Qtb,data):
        ...
        out = numerator/denominator  # [N, 20, 20]
    ...
    def sample_p_zs_given_zt(self,t,s,zt,data,cond,diverse,step,last_step, saved_dirs=None):
        """
        sample zs~p(zs|zt)
        """
        ...
        Qt = (Qsb/Qtb)/(Qsb/Qtb).sum(dim=-1).unsqueeze(dim=2) #approximate probality

        noise_data = data.clone()
        noise_data.x = zt 
        pred,_ = self.model(noise_data,t*self.timesteps, data.x, saved_dirs=saved_dirs)
    ...
    def sample(self,data,cond = False,temperature=1.0,stop = 0):
        ...
        zt = self.sample_discrete_feature_noise(limit_dist = limit_dist,num_node = data.x.shape[0]) 
        zt = zt.to(data.x.device)
        for s_int in tqdm(list(reversed(range(stop, self.timesteps)))):
            #z_t-1 ~p(z_t-1|z_t),
    ...
    def ddim_sample(self,data,cond = False,diverse=False,stop = 0,step=50, saved_dirs=None):
        limit_dist = torch.ones(20)/20
        zt = self.sample_discrete_feature_noise(limit_dist = limit_dist,num_node = data.x.shape[0]) 
        zt = zt.to(data.x.device)
        for s_int in tqdm(list(reversed(range(stop, self.timesteps,step)))):
            ...
            s_norm = s_array / self.timesteps
            t_norm = t_array /self.timesteps
            zt , final_predicted_X  = self.sample_p_zs_given_zt(t_norm, s_norm,zt, data,cond,diverse,step,last_step=s_int==stop, saved_dirs=saved_dirs)
        return zt,final_predicted_X
    ...
    def forward(self,data,logit=False):
        ...
        pred_X, attn_loss = self.model(noise_data,t_int, data.x)
        ...
        all_loss = ce_loss + attn_loss
        if logit:
            return ce_loss, pred_X
        else:
            return all_loss, ce_loss, attn_loss
...
class Trianer(object):
    def __init__(
        ...
        *,
        ...
        self.save_file_name = f"embe_noise={self.config['noise_type']}"
        self.step = 0
        
    def save(self, milestone, recovery_best=None):
        ...
        if self.step >= 60000:
            torch.save(data, os.path.join(str(self.results_folder), f'BestModel.pt'))
        ...
        with tqdm(initial = self.step, total = self.train_num_steps) as pbar:
            ...
            while self.step < self.train_num_steps:
                for _ in range(self.gradient_accumulate_every):
                    ...
                    loss,ce_loss, attn_loss = self.model(data)
                    # loss = self.model(data)
                    acc_loss = loss / self.gradient_accumulate_every
                    total_loss += loss.item()
                    acc_loss.backward()
                wandb.log({"train_loss": loss, "ce_loss": ce_loss,"attn_loss": attn_loss})
                torch.nn.utils. clip_grad_norm_(self.model.parameters(), 1.0)
                all_iter = len(self.ds)//64 
                ...
                self.ema.to(device)
                self.ema.update()
                if self.step != 0 and self.step % (self.save_and_sample_every*((len(self.ds)//3)//64)) == 0: 
                    ...
                    with torch.no_grad():
                        ...
                        for data in self.val_loader:
                            ...
                            prob,sample_graph = self.ema.ema_model.ddim_sample(data,diverse = False,step=500) 
                    ...
                    wandb.log({"recovery_rate": recovery_list[-1], "perplexity": perplexity[-1], "val_step": self.step})

                    if recovery_list[-1] == max(recovery_list):
                        print(f"current step {self.step} is saved, please check correspond recovery.")
                        self.save(milestone, recovery_list[-1])
...
if __name__ == "__main__" :
    ...
    parser.add_argument('--dataset', type = str,default='TS',help='the dataset to train on')
    ...
    diffusion_model = AGSDD(model,timesteps=config['timesteps'],objective=config['objective'],config=config)
